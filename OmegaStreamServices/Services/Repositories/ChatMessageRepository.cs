...
namespace OmegaStreamServices.Services.Repositories
{
    public class ChatMessageRepository : BaseRepository<ChatMessage>, IChatMessageRepository
    {
        ...
        {
            string lastMessage = await _dbSet.Where(x => x.UserChatId == chatId).OrderBy(x => x.SentAt)
                .Select(x => x.Content).LastOrDefaultAsync();
            if (lastMessage != null)
            {
                return _encryptionHelper.Decrypt(lastMessage);
            }
            return null;
        }
    }
